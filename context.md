...
- **Aprovar** → envia realment el correu via l'API del proveïdor (Gmail API / Microsoft Graph), amb opció d'editar el text i els destinataris (Per a / Cc / Cco) i d'adjuntar-hi documents (fins a 3 MB en total, el límit del més estricte dels dos proveïdors) abans d'aprovar. Els camps de destinataris arrenquen amb el que el fil implica (respondre a qui ha escrit) i autocompleten amb els contactes recents del tenant, llegits de les adreces que `messages` ja acumula — no hi ha taula de contactes. Els tres camps es desen a l'esborrany en aprovar-lo, perquè l'audit log tingui a qui es va enviar i no només què (§7). Els fitxers adjunts, en canvi, no es desen enlloc: viuen de l'enviament del formulari fins que el proveïdor té el correu, i de l'esborrany només en queda el nom a l'audit log.
